...
// +-------------------------------------------------------------------------+-
// | $Id: Facebook_Api_Core, v1.03 2020/09/01 12:23:00 Exp $
 ...
 */
class Core extends \App_Abstract_Socialnet_Core
{
    /** @var string Version number of the Facebook Bot PHP SDK. */
    const VERSION = '2.3.8';

    /** @var string The name of the environment variable that contains the Facebook Bot API Access Token. */
    const BOT_TOKEN_ENV_NAME = 'FACEBOOK_BOT_TOKEN';
     ...
     * @return self
     */
    public function setRecipient($items = array())
    {
        $this->recipient = new Recipient($items);
        return $this;
    }
...
_develop('_core->outputEntity BODY:'); _develop($body);
    ...
    public function commonEvent($event = null)
    {
        if (!$event instanceof Event) {
            ...
            $this->throwException('Event must be instance of \App\Socialnet\Facebook\Api\Event', 4);
        }
...
_develop('process event 1');
            // make event from json
            $event = Event\Factory::makeFromApi($eventBody);
_develop('process event 2');
            // Init User from Sender or Recipeint
            $event->setPageId( $this->account->getPageId() );
            $event->initUser();
_develop('process event 3');
