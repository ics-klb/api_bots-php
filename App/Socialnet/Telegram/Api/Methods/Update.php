...
use App\Socialnet\Telegram\Api\Events\UpdateWasReceived;
use App\Socialnet\Telegram\Api\Exceptions\CoreSDKException;
use App\Socialnet\Telegram\Api\FileUpload\InputFile;
use App\Socialnet\Telegram\Api\Objects\Update as UpdateObject;
use App\Socialnet\Telegram\Api\Objects\WebhookInfo;
use App\Socialnet\Telegram\Api\TelegramResponse;
...
trait Update
{
    ...
    private function validateHookUrl(string $url)
    {
        if (filter_var($url, FILTER_VALIDATE_URL) === false) {
            throw new CoreSDKException('Invalid URL Provided');
        }

        if (parse_url($url, PHP_URL_SCHEME) !== 'https') {
            throw new CoreSDKException('Invalid URL, should be a HTTPS url.');
        }
    }
